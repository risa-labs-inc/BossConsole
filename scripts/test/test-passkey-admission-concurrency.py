    ...
    try:
        ...
        try:
            ...
            assert sql("set role service_role; select allowed from "
                       "public.admit_passkey_challenge('authentication', 'untrusted');",
                       expected_error='55P03') is None
            assert time.monotonic() - started < 2, 'Admission waited for the held transaction'
            assert sql("set role service_role; select allowed from "
                       "public.admit_passkey_challenge('registration', 'untrusted');") == 't'
            print('Held authentication lock: bounded refusal, registration remains available')
        finally:
            holder.wait(timeout=10)
            holder.stdout.close()
            holder.stderr.close()
        sql("update public.passkey_challenge_admission set used=224, window_started_at=clock_timestamp() "
        ...
        assert sql(f"select count(*) from public.passkey_challenges where challenge like '{prefix}%';") == '0'
        sql("update public.passkey_challenge_admission set window_started_at=clock_timestamp()-interval '61 seconds' "
            "where type='authentication' and lane='untrusted';")
        assert sql("set role service_role; select allowed from "
                   "public.admit_passkey_challenge('authentication', 'untrusted');") == 't'
        print('Cleanup removed the expired batch, and legitimate retry succeeded after expiry')
    finally:
        sql(f"delete from public.passkey_challenges where challenge like '{prefix}%';")
        for row in previous:
            if row['type'] not in ('authentication', 'registration') or row['lane'] not in (
                    'untrusted', 'trusted') or not isinstance(row['used'], int):
            ...
            sql("update public.passkey_challenge_admission set "
                f"window_started_at='{stamp}'::timestamptz, used={row['used']} "
                f"where type='{row['type']}' and lane='{row['lane']}';")

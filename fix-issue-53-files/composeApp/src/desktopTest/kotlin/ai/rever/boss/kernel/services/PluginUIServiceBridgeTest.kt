...
import ai.rever.boss.ipc.auth.ProcessAuthClientInterceptor
import ai.rever.boss.ipc.auth.ProcessAuthRegistry
import ai.rever.boss.ipc.auth.ProcessAuthServerInterceptor
...
class PluginUIServiceBridgeTest {
    ...
    private lateinit var authRegistry: ProcessAuthRegistry
    private lateinit var server: Server
    private lateinit var channel: ManagedChannel
    private lateinit var plugin: PluginUIServiceGrpcKt.PluginUIServiceCoroutineStub
    private val authenticatedChannels = CopyOnWriteArrayList<ManagedChannel>()
    ...
    fun setUp() {
        ...
        authRegistry = ProcessAuthRegistry()
        server =
            ServerBuilder
                .forPort(0)
                // Installed unconditionally, including for every test above this one that never attaches
                // a token: an unauthenticated caller is exactly what a plugin launched outside
                // ProcessSpawner looks like, and this interceptor must be a no-op for it. Every test that
                // predates process auth passing unmodified is that guarantee, exercised for real.
                .intercept(ProcessAuthServerInterceptor(authRegistry))
    ...
    // ---- Process-identity tests (issue #53: StreamUI had no owner check) ----

    @Test
    fun `an unauthenticated registration is recorded unverified, exactly as before process auth existed`() =
        runBlocking {
            // Every test above this one makes exactly this kind of call. Pinning it explicitly here means
            // a future change to the default can be caught by one assertion instead of noticing sixteen
            // unrelated tests went red.
            assertTrue(plugin.registerUI(registration(PANEL)).success)

            assertFalse(registry.surfaceOf(PANEL)?.identityVerified ?: true)
        }

    @Test
    fun `a registration's self-reported process_id is ignored once the caller is authenticated`() =
        runBlocking {
            val real = authenticatedStub("plugin-real-identity")

            // Claims to be a different process entirely - the verified identity must win regardless of
            // what the request body says, or RegisterUI would still be spoofable through the field this
            // whole mechanism exists to stop trusting.
            assertTrue(real.registerUI(registration(PANEL, process = "plugin-someone-else")).success)

            assertEquals("plugin-real-identity", registry.surfaceOf(PANEL)?.processId)
            assertTrue(registry.surfaceOf(PANEL)?.identityVerified == true)
        }

    @Test
    fun `a forged token grants no identity`() =
        runBlocking {
            val forgedChannel =
                ManagedChannelBuilder
                    .forAddress("localhost", server.port)
                    .usePlaintext()
                    .intercept(ProcessAuthClientInterceptor("not-a-token-anyone-issued"))
                    .build()
            authenticatedChannels += forgedChannel
            val forged = PluginUIServiceGrpcKt.PluginUIServiceCoroutineStub(forgedChannel)

            assertTrue(forged.registerUI(registration(PANEL, process = "plugin-a")).success)

            // An unrecognised token is the same as no token: the caller falls back to the (unverifiable)
            // request field, exactly like every unauthenticated caller in this file - it does not, for
            // example, get attributed to whatever process the real token belongs to.
            assertEquals("plugin-a", registry.surfaceOf(PANEL)?.processId)
            assertFalse(registry.surfaceOf(PANEL)?.identityVerified ?: true)
        }

    @Test
    fun `a different authenticated process cannot bind another's surface with StreamUI`() =
        runBlocking {
            val owner = authenticatedStub("plugin-owner")
            val intruder = authenticatedStub("plugin-intruder")
            assertTrue(owner.registerUI(registration(PANEL, process = "plugin-owner")).success)

            val stolen = Channel<WidgetUpdate>(Channel.UNLIMITED)
            val attempt = intruder.streamUI(stolen.consumeAsFlow())
            stolen.send(fullTree(PANEL, label = "Stolen"))

            val failure = assertFailsWith<StatusException> { attempt.toList() }

            assertEquals(Status.Code.PERMISSION_DENIED, failure.status.code)
            assertContains(failure.status.description.orEmpty(), "different process")
            assertNull(registry.surfaceOf(PANEL)?.tree, "the intruder's tree must never have been applied")
            assertFalse(registry.surfaceOf(PANEL)?.streaming == true, "the intruder must not hold the claim")
        }

    @Test
    fun `a different authenticated process cannot UnregisterUI another's surface`() =
        runBlocking {
            val owner = authenticatedStub("plugin-owner")
            val intruder = authenticatedStub("plugin-intruder")
            assertTrue(owner.registerUI(registration(PANEL, process = "plugin-owner")).success)

            intruder.unregisterUI(UIUnregistration.newBuilder().setSurfaceId(PANEL).build())

            assertNotNull(registry.surfaceOf(PANEL), "an intruder's UnregisterUI must not remove it")
        }

    @Test
    fun `the owning authenticated process can still stream and unregister its own surface`() =
        runBlocking {
            val owner = authenticatedStub("plugin-owner")
            assertTrue(owner.registerUI(registration(PANEL, process = "plugin-owner")).success)
            val updates = Channel<WidgetUpdate>(Channel.UNLIMITED)
            ...
            coroutineScope {
                val stream = launch { runCatching { owner.streamUI(updates.consumeAsFlow()).toList() } }
                updates.send(fullTree(PANEL))
                awaitTrue { registry.surfaceOf(PANEL)?.streaming == true }

                owner.unregisterUI(UIUnregistration.newBuilder().setSurfaceId(PANEL).build())
                stream.join()
            }
    ...
    /**
     * A stub on its own channel, authenticated as [processId] via a freshly issued token.
     *
     * Its own channel because [ProcessAuthClientInterceptor] is installed per-channel, standing in for a
     * separate process with its own spawn-time token — which is what [processId] actually is once this
     * returns. The channel is tracked for [tearDown] rather than closed here, since a caller usually still
     * needs the stub afterwards.
     */
    private fun authenticatedStub(processId: String): PluginUIServiceGrpcKt.PluginUIServiceCoroutineStub {
        val token = authRegistry.issueToken(processId)
        val authChannel =
            ManagedChannelBuilder
                .forAddress("localhost", server.port)
                .usePlaintext()
                .intercept(ProcessAuthClientInterceptor(token))
                .build()
        authenticatedChannels += authChannel
        return PluginUIServiceGrpcKt.PluginUIServiceCoroutineStub(authChannel)
    }

...
class KernelBootstrap(
    ...
    fun initialize() {
        ...
        ipcServer =
            BossIpcServer(
                kernelAddress!!,
                // The one server every plugin connects to, and therefore the one that needs to know who
                // is actually calling rather than trusting what a request body claims - see
                // PluginUIServiceBridge and the RemoteUiSurfaceRegistry.openStream/unregister call sites
                // it feeds a verified identity to. ProcessSpawner mints the tokens this verifies against
                // ProcessAuthRegistry.shared by default, which is what makes this a one-line kernel-side
                // change rather than a wire-up through every call site that spawns a process.
                interceptors = listOf(ProcessAuthServerInterceptor(ProcessAuthRegistry.shared)),

...
class RemoteUiSurface internal constructor(
    ...
    /**
     * Whether [processId] came from an authenticated caller rather than a self-reported request field.
     *
     * `RegisterUI`'s `process_id` is written by the plugin process itself, so trusting it is exactly the
     * gap `ProcessAuthServerInterceptor` closes - but only for a caller whose launcher attaches a spawn
     * token, which is a rollout, not a flag day. `false` here means [processId] is only ever a claim: the
     * registry falls back to today's behaviour and does not compare it against later callers. `true` means
     * it was read from a verified identity, and `openStream`/`unregister` may enforce that a later caller's
     * own verified identity matches it - see `RemoteUiSurfaceRegistry.openStream`.
     */

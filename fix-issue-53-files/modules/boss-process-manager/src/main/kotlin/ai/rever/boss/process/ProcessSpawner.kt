 ...
 * - BOSS_PROCESS_TOKEN: This process's identity token - see [ProcessAuthRegistry]
 *
...
class ProcessSpawner(
    ...
    /**
     * Where a spawned process's identity token is minted and recorded.
     *
     * Defaults to the same shared instance `ProcessAuthServerInterceptor` reads from on the kernel's
     * server, so every real caller gets working authentication with no wiring beyond this default. Tests
     * that want an isolated token space pass their own.
     */
    ...
    fun spawn(config: ProcessConfig): ManagedProcess {
        ...
        // Minted before the process exists, so there is no window where a connection could arrive before
        // the kernel-side interceptor knows what this token means. See ProcessAuthRegistry.
        val authToken = processAuthRegistry.issueToken(config.processId)
        ...
        processBuilder.environment().apply {
            ...
            put("BOSS_PROCESS_TOKEN", authToken)

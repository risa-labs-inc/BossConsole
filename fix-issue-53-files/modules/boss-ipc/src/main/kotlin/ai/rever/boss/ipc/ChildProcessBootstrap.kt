...
class ChildProcessBootstrap {
    ...
    /**
     * This process's spawn-time identity token, if `ProcessSpawner` issued one.
     *
     * Unlike [processId]/[processType]/[kernelAddress], a missing token is not fatal: a process launched
     * outside `ProcessSpawner` - or one from before this existed - simply has none, and every call it
     * makes to the kernel is then treated as unverified, exactly as every call was treated before this
     * existed. See `ProcessAuthRegistry`.
     */
    val processToken: String? by lazy { System.getenv("BOSS_PROCESS_TOKEN") }
    ...
    suspend fun connect(
        ...
        val kernelClient =
            BossIpcClient(
                kernelAddress,
                interceptors = processToken?.let { listOf(ProcessAuthClientInterceptor(it)) } ?: emptyList(),
            )

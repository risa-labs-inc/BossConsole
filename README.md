...
- **A session-wide brake for agent actions.** Open the clickable **MCP** status in the bottom bar and choose **Pause Actions** to block every new mutating tool call across all connected agents while read-only inspection keeps working. Already-running mutations are counted and allowed to finish; blocked attempts are audited. Resuming is one click and does not rewrite saved policies or per-tool kill-switches.

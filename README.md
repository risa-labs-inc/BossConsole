...
- **Flight Plan before execution.** Open the clickable **MCP** status in the bottom bar, then choose **Flight Plan** to preview a registered action's current route through the kill-switch, access gate, policy, and human-approval gate. It shows whether the handler would run, wait for you, or be withheld - without invoking the tool, opening an approval request, or writing to the audit ledger.

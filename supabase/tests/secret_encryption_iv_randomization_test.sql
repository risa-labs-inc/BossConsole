...
-- deterministic, zero-IV ciphertext. All migrations are already applied by the
-- time this file runs, so 20260914000000_randomize_secret_encryption_iv.sql's
-- own backfill has nothing to convert on a fresh database; the last block
-- below re-runs its exact UPDATE statement against a row inserted in the old
-- format to prove that statement's logic directly, rather than only trusting
...
SELECT matches(public.encrypt_text('round-trip-me'), '^v2:', 'new ciphertext carries the v2 envelope');
SELECT is(
    public.decrypt_text(public.encrypt_text('round-trip-me')),
    'round-trip-me',
    'a v2-encrypted value round trips'
);
...
-- ---- The backfill statement converts a legacy row to the v2 envelope, and
-- is idempotent (a second run leaves an already-migrated row unchanged).
...
SELECT matches(
    ...
    '^v2:',
    'the backfill statement upgrades a legacy row to the v2 envelope'
);
SELECT is(

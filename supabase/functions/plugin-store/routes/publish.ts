...
publish.openapi(publishFromGitHubMetadataRoute, async (ctx) => {
  try {
    ...
    // Reject private repos on the metadata path. Unlike POST /github (which
    // re-hosts the JAR in Supabase Storage), this path stores the GitHub
    // download URL as the client-facing jar_path and never buffers the bytes —
    // so a token-less client could not install a private-repo plugin, and the
    // server-side manifest/hash steps below fetch the unauthenticated CDN URL,
    // which 404s for a private repo. Large (>50 MB) private plugins are simply
    // not supported here; fail fast with an actionable message instead of the
    // later 404-shaped failure.
    ...
    if (repoIsPrivate) {
      return ctx.json({
        success: false,
        error: `Private repositories are not supported on the metadata (large-JAR) publish path: ` +
          `it stores the GitHub download URL for direct client download, which requires a public repo. ` +
          `Make ${parsed.owner}/${parsed.repo} public, or keep the plugin under 50 MB so it can be published ` +
          `via POST /github (which re-hosts the JAR in the store).`
      }, 400)
    }

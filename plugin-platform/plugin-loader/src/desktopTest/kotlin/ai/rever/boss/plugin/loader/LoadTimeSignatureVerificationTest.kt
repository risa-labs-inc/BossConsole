...
class LoadTimeSignatureVerificationTest {
    ...
    private fun testLoader() = DynamicPluginLoaderImpl(
    ...
    private fun manifestJar(pluginId: String, version: String): File {
    ...
    private fun writeSidecar(jar: File, pluginId: String, version: String, sha256: String) {
    ...
    @Test
    fun `valid sidecar passes the signature gate`() = runBlocking<Unit> {
    ...
    @Test
    fun `sidecar signed for a different version is rejected`() = runBlocking<Unit> {
    ...
    @Test
    fun `tampered jar with a valid-looking sidecar is rejected`() = runBlocking<Unit> {
    ...
    @Test
    fun `missing sidecar is allowed during rollout`() = runBlocking<Unit> {
    ...
    @Test
    fun `missing sidecar hard-fails once enforcement is on`() = runBlocking<Unit> {
    ...
    @Test
    fun `missing sidecar stays allowed in dev mode even under enforcement`() = runBlocking<Unit> {
    ...
    @Test
    fun `dev mode does not rescue a tampered sidecar`() = runBlocking<Unit> {

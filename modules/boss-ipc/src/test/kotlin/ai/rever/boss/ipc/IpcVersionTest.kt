...
class IpcVersionTest {
    ...
    fun `authenticated runtime minimum refuses pre-authentication hosts`() {
        assertEquals("1.3.0", IpcVersion.CURRENT)
        assertTrue(IpcVersion.isCompatible("1.3.0", "1.2.0") is IpcVersion.CompatResult.Incompatible)
        assertEquals(IpcVersion.CompatResult.Compatible, IpcVersion.isCompatible("1.2.0", "1.3.0"))
    }

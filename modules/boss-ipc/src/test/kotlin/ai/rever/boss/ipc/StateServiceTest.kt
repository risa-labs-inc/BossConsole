...
class StateServiceTest {
    ...
    @Test
    fun `getState returns empty for unknown key`() = runBlocking {
    ...
    @Test
    fun `setState then getState returns stored value`() = runBlocking {
        val stub = StateServiceGrpcKt.StateServiceCoroutineStub(channel!!)
    ...
    @Test
    fun `watchState emits current value then updates`() = runBlocking {
        withTimeout(10_000) {
            val stub = StateServiceGrpcKt.StateServiceCoroutineStub(channel!!)
    ...
    @Test
    fun `setState increments version on each update`() = runBlocking {
        val stub = StateServiceGrpcKt.StateServiceCoroutineStub(channel!!)

        val v1 = stub.setState(
            StateUpdate.newBuilder()
    ...
    @Test
    fun `setState with wrong expectedVersion returns current without updating`() = runBlocking {
        val stub = StateServiceGrpcKt.StateServiceCoroutineStub(channel!!)

        // Set initial
        val initial = stub.setState(
            StateUpdate.newBuilder()
    ...
    @Test
    fun `listStateKeys returns all stored keys`() = runBlocking {
        val stub = StateServiceGrpcKt.StateServiceCoroutineStub(channel!!)

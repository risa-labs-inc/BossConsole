...
class MasteryServiceImpl(
    ...
    override fun executeMastery(request: ExecuteMasteryRequest): Flow<PProgress> = flow {
        val def = definitions[request.masteryId] ?: run {
    ...
    override suspend fun cancelMastery(request: MasteryExecutionId): CancelMasteryResponse {
        ...
        return if (job != null) {
            ...
            CancelMasteryResponse.newBuilder()
                .setSuccess(true).setMessage("Execution ${request.executionId} cancelled").build()
        } else {
            CancelMasteryResponse.newBuilder()
                .setSuccess(false).setMessage("No running execution: ${request.executionId}").build()
        }
    }

    override suspend fun getMasteryStatus(request: MasteryExecutionId): MasteryStatus =
        execStatus[request.executionId]
            ?: MasteryStatus.newBuilder().setExecutionId(request.executionId).setState("unknown").build()
    ...
    override suspend fun listMasteries(request: ListMasteriesRequest): ListMasteriesResponse {
        val all: List<KMasteryDef> = definitions.values
            .filter { def ->
        ...
        val paginated = if (request.limit > 0) all.drop(request.offset).take(request.limit)
                        else all.drop(request.offset)

        return ListMasteriesResponse.newBuilder()
            .addAllMasteries(paginated.map { def ->
                MasterySummary.newBuilder()
                    .setId(def.id).setName(def.name).setDescription(def.description)
                    .setNodeCount(def.nodes.size).setAuthor(def.author).build()
            })
    ...
    private fun updateStatus(executionId: String, masteryId: String, state: String) {
        val builder = execStatus[executionId]?.toBuilder()
            ?: MasteryStatus.newBuilder()
...
private fun PMasteryDef.toKotlin(): KMasteryDef =
    KMasteryDef(
        ...
        inputSchemaJson = inputSchemaJson, outputSchemaJson = outputSchemaJson,
        nodes = nodesList.map { n ->
            KMasteryNode(
                id = n.id, pluginId = n.pluginId, action = n.action,
                inputMapping = n.inputMappingMap, staticConfig = n.staticConfigMap,
        ...
        edges = edgesList.map { e ->
            KMasteryEdge(
                ...
                outputKey = e.outputKey, inputKey = e.inputKey,
...
private fun KMasteryDef.toProto(): PMasteryDef {
    val b = PMasteryDef.newBuilder()
        .setId(id).setName(name).setDescription(description)
        .setInputSchemaJson(inputSchemaJson).setOutputSchemaJson(outputSchemaJson)
        .setAuthor(author).setCreatedAt(createdAt).setUpdatedAt(updatedAt)
    nodes.forEach { n ->
        b.addNodes(
            PMasteryNode.newBuilder()
                .setId(n.id).setPluginId(n.pluginId).setAction(n.action)
                .putAllInputMapping(n.inputMapping).putAllStaticConfig(n.staticConfig)
                .setIsAgentCall(n.isAgentCall)
                .apply { n.agentPrompt?.let { setAgentPrompt(it) } }
                .setMaxRetries(n.maxRetries).setTimeoutMs(n.timeoutMs).setDisplayName(n.displayName)
                .build()
        )
    }
    edges.forEach { e ->
        b.addEdges(
            PMasteryEdge.newBuilder()
                .setFromNode(e.fromNode).setToNode(e.toNode)
                .setOutputKey(e.outputKey).setInputKey(e.inputKey)
...
private fun KProgress.toProto(executionId: String): PProgress {
    val b = PProgress.newBuilder()
        .setExecutionId(executionId)
        .setTimestamp(System.currentTimeMillis())
    when (this) {
        is KProgress.Started ->
            b.setStarted(MasteryStarted.newBuilder().setMasteryId(masteryId).setTotalNodes(totalNodes).build())
        is KProgress.NodeStarted ->
            b.setNodeStarted(NodeStarted.newBuilder().setNodeId(nodeId).setDisplayName(displayName).build())
        is KProgress.NodeCompleted ->
            b.setNodeCompleted(NodeCompleted.newBuilder().setNodeId(nodeId).putAllOutput(output).setDurationMs(durationMs).build())
        is KProgress.NodeFailed ->
            b.setNodeFailed(NodeFailed.newBuilder().setNodeId(nodeId).setErrorMessage(error).setWillRetry(willRetry).build())
        is KProgress.Completed ->
            b.setCompleted(MasteryCompleted.newBuilder().putAllOutput(output).setTotalDurationMs(totalDurationMs).build())
        is KProgress.Failed ->
            b.setFailed(MasteryFailed.newBuilder().setErrorMessage(error).setFailedNodeId(failedNodeId).build())
    }
    return b.build()
}

...
class TerminalPipeFailureTest {
    @Test
    fun `real process pipe timeout and subsequent EOF do not wait for blocked close`() {
        val command =
            listOf(
        ...
        val session = TerminalSession("real-pipe", "/fixture", command, process, 80, 24, inputWriteTimeoutMillis = 100)
        // The outer deadline runs independently of the RPC thread: a blocking native close
        // cannot be interrupted by a coroutine timeout on that same thread.
        val caller =
            Thread {
                try {
                    assertEquals("ready", process.inputStream.bufferedReader().readLine())
                    runBlocking {
                        val error = assertFailsWith<StatusRuntimeException> { session.send(ByteArray(1024 * 1024)) }
                        assertEquals(Status.Code.ABORTED, error.status.code)
                        val retry = assertFailsWith<StatusRuntimeException> { session.send(byteArrayOf(1)) }
                        assertEquals(Status.Code.FAILED_PRECONDITION, retry.status.code)
                        session.closeStdin()
                        session.closeStdin()
                        assertTrue(process.isAlive, "Timeout must not kill the user's process")
                    }

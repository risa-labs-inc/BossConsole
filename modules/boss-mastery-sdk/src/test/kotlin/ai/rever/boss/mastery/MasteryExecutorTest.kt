...
class MasteryExecutorTest {
    ...
    @Test
    fun `simple two-node sequential mastery executes both nodes in order`() =
        runBlocking {
            val resolver =
                MockResolver(
                    mapOf(
    ...
    @Test
    fun `data passes between nodes via inputMapping`() =
        runBlocking {
            val resolver =
                MockResolver(
                    mapOf(
            ...
            assertEquals(
                "generated text",
                sumCall.third["content"],
                "Expected 'content' input to be forwarded from gen.text",
            )
        }

    @Test
    fun `progress events emitted in correct order for single node`() =
        runBlocking {
            val resolver =
                MockResolver(
    ...
    @Test
    fun `INPUT data available to first node via inputMapping`() =
        runBlocking {
            val resolver =
                MockResolver(

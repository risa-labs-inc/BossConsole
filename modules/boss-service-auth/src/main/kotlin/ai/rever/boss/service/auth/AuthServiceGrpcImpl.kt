...
class AuthServiceGrpcImpl(
    ...
    override fun watchAuthState(request: Empty): Flow<AuthStateResponse> =
        flow {
            emit(
                AuthStateResponse
                    .newBuilder()
    ...
    override suspend fun signIn(request: SignInRequest): SignInResponse {
        ...
        val client =
            supabaseClient ?: return SignInResponse
                .newBuilder()
                .setSuccess(false)
    ...
    private suspend fun handleMagicLink(
        client: SupabaseAuthClient,
        email: String,
    ): SignInResponse =
        when (val result = client.sendMagicLink(email)) {
            is AuthResult.MagicLinkSent -> {
                SignInResponse
                    .newBuilder()
                    .setSuccess(true)
    ...
    override fun watchCurrentUser(request: Empty): Flow<UserInfoResponse> =
        flow {
            emit(
                UserInfoResponse
                    .newBuilder()

...
await as(a);
assert.deepEqual(await scalar("SELECT get_user_terminal_preferences() value"), {
  unfocused_mode: "batch",
  unfocused_fps: 4,
  revision: 0,
});
assert.equal(
  (await scalar("SELECT set_user_terminal_preferences('preview',10,0) value"))
    .revision,
  1,
);
...
const handoff = await scalar("SELECT mint_user_settings_handoff($1) value", [
  a,
]);
...
await as(b);
assert.equal(
  (await scalar("SELECT get_user_terminal_preferences() value")).revision,
  0,
);
assert.equal(
...
assert.equal(
  (await scalar("SELECT consume_user_settings_handoff($1) value", [
    handoff.token,
  ])).user_id,
  a,
);
...
assert.equal(
  (await scalar("SELECT consume_terminal_relay_ticket($1,$2) value", [
    ticket.ticket,
    room,
  ])).role,
  "host",
...
assert.equal(
  (await scalar("SELECT get_user_terminal_preferences($1) value", [a]))
    .unfocused_fps,
  10,
);
...
await as(a);
await scalar("SELECT mint_terminal_relay_ticket($1,'host') value", [room]);
await db.exec("RESET ROLE");
...
await as(a);
await assert.rejects(db.query("SELECT mint_terminal_relay_ticket($1,'host')", [foreignRoom]), e => e.code === "42501");
await db.exec("RESET ROLE");
...
await as(a);
await assert.rejects(db.query("SELECT mint_terminal_relay_ticket($1,'account')", [room]), e => e.code === "54000");
await db.exec("RESET ROLE");
await db.query("UPDATE terminal_relay_tickets SET expires_at=now() WHERE room_id=$1", [room]);
await as(a);
const renewed = await scalar("SELECT mint_terminal_relay_ticket($1,'account') value", [room]);
await db.exec("RESET ROLE");
...
await as("", "service_role");
await assert.rejects(db.query("SELECT consume_terminal_relay_ticket($1,$2)", [renewed.ticket,room]), e => e.code === "42501");
await as(a);
await scalar("SELECT mint_terminal_relay_ticket($1,'host') value", [room]);
...
await as(a);
await assert.rejects(db.query("SELECT mint_terminal_relay_ticket($1,'host')", ["55555555-5555-4555-8555-555555555555"]), e => e.code === "54000");
await scalar("SELECT mint_terminal_relay_ticket($1,'host') value", [room]);
...
await as(a);
await scalar("SELECT mint_user_settings_handoff() value");
await db.exec("RESET ROLE");
...
await as(a);
await assert.rejects(db.query("SELECT mint_user_settings_handoff()"), e => e.code === "54000");
await db.exec("RESET ROLE");

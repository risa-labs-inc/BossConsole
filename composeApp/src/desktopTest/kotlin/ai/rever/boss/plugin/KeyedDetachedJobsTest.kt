...
class KeyedDetachedJobsTest {
    ...
    @Test
    fun `job runs to completion although the caller is cancelled mid-run`() = runBlocking {
    ...
    @Test
    fun `concurrent runs for the same key coalesce onto one execution`() = runBlocking {
        val owner = ownerScope()
        val jobs = KeyedDetachedJobs<String, String>(owner)
    ...
    @Test
    fun `a run after completion executes again instead of joining a stale result`() = runBlocking {
        val owner = ownerScope()
        val jobs = KeyedDetachedJobs<String, String>(owner)
        val executions = AtomicInteger()

        assertEquals("a", jobs.run("plugin") { executions.incrementAndGet(); "a" })
        assertEquals("b", jobs.run("plugin") { executions.incrementAndGet(); "b" })
        assertEquals(2, executions.get())
        owner.cancel()
    }

    @Test
    fun `distinct keys run independently`() = runBlocking {
        val owner = ownerScope()
        val jobs = KeyedDetachedJobs<String, String>(owner)
        val gate = CompletableDeferred<Unit>()
        val entered = CompletableDeferred<Unit>()
    ...
    @Test
    fun `detached failure after caller cancellation reaches onDetachedFailure`() = runBlocking {
        val owner = ownerScope()
        val jobs = KeyedDetachedJobs<String, String>(owner)
        val entered = CompletableDeferred<Unit>()
        val gate = CompletableDeferred<Unit>()

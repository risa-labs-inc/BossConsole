...
class PluginStoragePersistenceTest {
    ...
    fun `failed writes leave the committed value unchanged and report failure`() =
        runBlocking {
            PluginStorageProviderImpl("test", directory).putString("key", "committed")
            val before = file.readBytes()
            val storage = PluginStorageProviderImpl("test", directory) { _, _ -> throw IOException("disk unavailable") }
    ...
    fun `readers do not see a mutation before its disk commit`() =
        runBlocking {
            ...
            val storage =
                PluginStorageProviderImpl("test", directory) { target, properties ->
                    entered.countDown()
            ...
            try {
                ...
                assertEquals("committed", storage.getString("key"))
                assertEquals("committed", PluginStorageProviderImpl("test", directory).getString("key"))
            } finally {
                release.countDown()
                write.await()
            }
            assertEquals("next", storage.getString("key"))
            assertEquals("next", PluginStorageProviderImpl("test", directory).getString("key"))
        }
    ...
    fun `typed and escaped values survive reload in the existing properties format`() =
        runBlocking {
            val storage = PluginStorageProviderImpl("test", directory)
            val text = "line one\nUnicode: \u00e9\u6f22\ud83d\ude00\\=:\t"
            ...
            val reloaded = PluginStorageProviderImpl("test", directory)

...
class AgentTraceStoreTest {
    ...
    @Test
    fun `startTrace creates a RUNNING event`() =
        runBlocking {
    ...
    @Test
    fun `completeTrace transitions to SUCCESS and calculates duration`() =
        runBlocking {
    ...
    @Test
    fun `completeTrace transitions to FAILURE on error result`() =
        runBlocking {
            val id = AgentTraceStore.startTrace("test.tool", "{}")
    ...
    @Test
    fun `failTrace transitions to TIMEOUT on timeout flag`() =
        runBlocking {
            val id = AgentTraceStore.startTrace("test.tool", "{}")
    ...
    @Test
    fun `failTrace transitions to CANCELLED on isCancelled flag`() =
        runBlocking {
            val id = AgentTraceStore.startTrace("test.tool", "{}")
    ...
    @Test
    fun `store truncates history at MAX_EVENTS`() =
        runBlocking {
            for (i in 1..501) {
    ...
    @Test
    fun `sanitization redacts sensitive payloads`() =
        runBlocking {

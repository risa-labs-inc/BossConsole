...
object McpMissionControlToolProvider : McpToolProvider {
    ...
    private fun historyTool(): McpToolDefinition {
        return McpToolDefinition(
            ...
            description = "Get the execution history of recent MCP tool calls in BOSS. " +
            ...
            handler = McpToolHandler { args ->
                ...
                val filtered = records.asSequence()
                    .filter { record ->
                        if (statusFilter.isNullOrBlank()) true
                        else record.status.name.equals(statusFilter, ignoreCase = true)
                    }
    ...
    private fun diagnoseTool(): McpToolDefinition {
        return McpToolDefinition(
            ...
            description = "Analyze the most recent failed, denied, or timed-out MCP tool call. " +
                "Returns specific guidance and cause diagnosis for autonomous recovery.",
            handler = McpToolHandler {
                val records = McpTelemetryRecorder.records.value
                val lastFailure = records.firstOrNull { r ->
                    r.status in setOf(
                        McpCallStatus.ERROR,
                ...
                if (lastFailure == null) {
                    return@McpToolHandler McpToolResult("No recent tool failures found. All recent calls completed successfully.")
                }

                val diagnosis = when (lastFailure.status) {
                    McpCallStatus.DENIED -> "The operator actively denied this action via the Human-in-the-Loop approval gate. " +
                        "Do not retry the exact same destructive command without explaining your intent to the user."
                    McpCallStatus.TIMEOUT -> "The tool exceeded its execution deadline (${lastFailure.durationMs}ms). " +
                        "If running a heavy build or network request, break it down or run asynchronously."
                    McpCallStatus.BLOCKED -> "The tool is currently disabled in the operator's kill-switch or requires elevated permissions. " +
                        "Ask the operator to enable '${lastFailure.toolName}' in Toolbox -> MCP."
                    McpCallStatus.ERROR -> "The tool handler returned an error. Verify the argument types and payload syntax."
                    else -> "Check error message details."
                }

                val result = DiagnosticReport(

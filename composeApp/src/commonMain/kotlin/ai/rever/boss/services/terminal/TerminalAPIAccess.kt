...
object TerminalAPIAccess {
     ...
     */
    fun hasTerminalState(
        windowId: String,
        terminalId: String,
    ): Boolean = getProvider()?.hasTerminalState(windowId, terminalId) ?: false

    /**
     * Remove terminal state for the given window and terminal ID.
     */
    fun removeTerminalState(
        windowId: String,
        terminalId: String,
    ) {
     ...
     */
    fun removeAllForWindow(windowId: String): Int = getProvider()?.removeAllForWindow(windowId) ?: 0

    /**
     * Reset all terminal states.
     */
    fun resetAllTerminals(): Int = getProvider()?.resetAllTerminals() ?: 0
    ...
    fun sendCommand(
        ...
        terminalId: String,
        command: String,
    ): Boolean = getProvider()?.sendCommand(windowId, terminalId, command) ?: false
    ...
    fun sendInterrupt(
        windowId: String,
        terminalId: String,
    ): Boolean = getProvider()?.sendInterrupt(windowId, terminalId) ?: false
    ...
    fun sendInput(
        ...
        terminalId: String,
        bytes: ByteArray,
    ): Boolean = getProvider()?.sendInput(windowId, terminalId, bytes) ?: false
    ...
    fun closeActiveTab(
        windowId: String,
        terminalId: String,
    ): Boolean = getProvider()?.closeActiveTab(windowId, terminalId) ?: false
    ...
    ): Boolean = getProvider()?.newSidebarTab(windowId, command, workingDirectory, configId, isRerun) ?: false

    /**
     * Register a sidebar tab ID for a config.
     */
    fun registerSidebarTabId(
        windowId: String,
        configId: String,
        tabId: String,
    ) {
     ...
     */
    fun removeSidebarConfigTracking(
        windowId: String,
        configId: String,
    ) {
     ...
     */
    fun getConfigIdForSidebarTab(
        windowId: String,
        tabId: String,
    ): String? = getProvider()?.getConfigIdForSidebarTab(windowId, tabId)

    /**
     * Set a pending sidebar command.
     */
    fun setPendingSidebarCommand(
        windowId: String,
        command: String,
     ...
     */
    fun consumePendingSidebarCommand(windowId: String): PendingSidebarCommand? = getProvider()?.consumePendingSidebarCommand(windowId)
    ...
    private fun wireRunnerCallbacks(api: TerminalTabPluginAPI) {
        try {
            ...
            terminalRemovedField.invoke(api, { windowId: String, terminalId: String ->
                try {
                    ai.rever.boss.run.RunnerTerminalService
                        .removeTerminal(windowId, terminalId)
                } catch (e: Exception) {
            ...
            configRemovedField.invoke(api, { windowId: String, configId: String ->
                try {
                    ai.rever.boss.run.RunnerTerminalService
                        .removeConfig(windowId, configId)
                } catch (e: Exception) {

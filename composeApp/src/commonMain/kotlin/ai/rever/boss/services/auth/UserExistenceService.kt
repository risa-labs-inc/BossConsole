                    ...
                    if (challengeResult.isSuccess) {
                        ...
                        challenge?.allowCredentials?.map { cred ->
                            ...
                            val credType = when {
                                ...
                                cred.transports?.contains("usb") == true && !cred.transports.contains("internal") -> WebAuthnCredentialType.USB_KEY
                                // Check for NFC security keys  
                                cred.transports?.contains("nfc") == true && !cred.transports.contains("internal") -> WebAuthnCredentialType.NFC_KEY
                                // Cross-device/hybrid credentials (not Touch ID)
                                cred.transports?.contains("hybrid") == true && !cred.id.contains("touchid-credential") -> WebAuthnCredentialType.CROSS_DEVICE

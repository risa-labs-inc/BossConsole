...
class UserManagementProviderImpl : UserManagementProvider {
    ...
    override suspend fun getAllUsersWithRoles(limit: Int, offset: Int): Result<PaginatedUsersData> {
        return UserService.getAllUsersWithRoles(limit, offset).map { paginated ->
            PaginatedUsersData(
    ...
    override suspend fun searchUsersByEmail(query: String, limit: Int, offset: Int): Result<PaginatedUsersData> {
        return UserService.searchUsersByEmail(query, limit, offset).map { paginated ->
            PaginatedUsersData(
                data = paginated.data.map { user ->
                    UserWithRolesData(
                        id = user.userId,
    ...
    override suspend fun assignRole(userId: String, roleName: String): Result<Unit> {
        return RoleService.assignRoleByName(userId, roleName)
    }

    override suspend fun removeRole(userId: String, roleName: String): Result<Unit> {
        return RoleService.removeRoleByName(userId, roleName)
    }

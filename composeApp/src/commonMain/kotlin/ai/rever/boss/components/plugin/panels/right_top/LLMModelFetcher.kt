...
object LLMModelFetcher {
    ...
    fun getModelsForProvider(provider: LLMProvider): List<DynamicLLMModel> {
        return _availableModels.value[provider.name] ?: getFallbackModelsForProvider(provider)
    }
    ...
    fun findModelById(modelId: String): DynamicLLMModel? {
        return _availableModels.value.values.flatten().find { it.id == modelId }
    }
    ...
    private fun isCacheExpired(cache: ModelCache): Boolean {
        return Clock.System.now().toEpochMilliseconds() - cache.lastUpdated > CACHE_DURATION_MS
    }

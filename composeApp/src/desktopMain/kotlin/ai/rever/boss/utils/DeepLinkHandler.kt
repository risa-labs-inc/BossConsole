...
actual object DeepLinkHandler {
    ...
    private fun setupWindowsHandler() {
        ...
        try {
            ...
            // present, command value missing — a failed .reg import can leave that shape) was
            // reported as "already registered" and never repaired, leaving boss:// broken for
            // that user on every launch. Costs one extra `reg query` per Windows start in the
            // already-correct case, bounded at 5s.

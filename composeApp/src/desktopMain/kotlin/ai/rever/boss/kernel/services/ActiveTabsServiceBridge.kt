...
class ActiveTabsServiceBridge(
    ...
    override fun watchActiveTabs(request: Empty): Flow<ActiveTabListResponse> = flow {
        provider.activeTabs.collect { tabs ->
            emit(
                ActiveTabListResponse.newBuilder()
                    .addAllTabs(tabs.map { tab ->
                        ActiveTabProto.newBuilder()
                            .setTabId(tab.tabId)
    ...
    override suspend fun getTabUrl(request: TabIdRequest): ActiveTabStringResponse {
        ...
        return ActiveTabStringResponse.newBuilder()
            .setValue(url ?: "")
            .build()
    }
    ...
    override suspend fun closeTab(request: TabIdRequest): ActiveTabBoolResponse {
        ...
        return ActiveTabBoolResponse.newBuilder()
            .setValue(success)
            .build()
    }
}

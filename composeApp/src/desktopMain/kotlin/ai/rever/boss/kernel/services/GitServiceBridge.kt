...
class GitServiceBridge(
    ...
    override suspend fun stage(request: GitFilePathRequest): GitOperationResultProto = provider.stage(request.path).toProto()

    override suspend fun unstage(request: GitFilePathRequest): GitOperationResultProto = provider.unstage(request.path).toProto()

    override suspend fun stageAll(request: Empty): GitOperationResultProto = provider.stageAll().toProto()

    override suspend fun unstageAll(request: Empty): GitOperationResultProto = provider.unstageAll().toProto()

    override suspend fun discardChanges(request: GitFilePathRequest): GitOperationResultProto =
        provider.discardChanges(request.path).toProto()

    override suspend fun cherryPick(request: GitHashRequest): GitOperationResultProto = provider.cherryPick(request.hash).toProto()

    override suspend fun revert(request: GitHashRequest): GitOperationResultProto = provider.revert(request.hash).toProto()

    override suspend fun checkout(request: GitRefRequest): GitOperationResultProto = provider.checkout(request.ref).toProto()

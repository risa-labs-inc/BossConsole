...
class LogServiceBridge(
    ...
    override fun watchLogs(request: Empty): Flow<LogListResponse> = flow {
        provider.logs.collect { logs ->
            emit(
                LogListResponse.newBuilder()
                    .addAllEntries(logs.map { entry ->
    ...
    override fun watchFilter(request: Empty): Flow<LogFilterProto> = flow {
        provider.filter.collect { filter ->
            emit(
                LogFilterProto.newBuilder()
                    .setFilter(
                        when (filter) {
    ...
    override fun watchSearchQuery(request: Empty): Flow<LogStringResponse> = flow {
        provider.searchQuery.collect { query ->
            emit(LogStringResponse.newBuilder().setValue(query).build())
        }
    }

    override fun watchAutoScroll(request: Empty): Flow<LogBoolResponse> = flow {
    ...
    override suspend fun setFilter(request: LogFilterProto): Empty {
        val filter = when (request.filter) {

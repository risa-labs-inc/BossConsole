...
object GitHubConfig {
    ...
    private fun validateToken(token: String): Boolean =
        when {
            ...
            // OAuth (gh CLI)
            token.startsWith("ghu_") -> token.length >= 40

            // User-to-server
            token.startsWith("ghs_") -> token.length >= 40

            // Server-to-server
            token.startsWith("ghr_") -> token.length >= 40

            // Refresh token
            token.startsWith("github_pat_") -> token.length >= 93

...
actual object PluginUpdateBridge {
    ...
    actual suspend fun checkOne(ref: InstalledPluginRef): UpdateCheckOutcome {
        val mgr = PluginStoreSetup.updateManager
        ...
        return try {
            ...
            when {
                ...
                available != null -> {
                    ...
                    if (incompatible != null) UpdateCheckOutcome.Incompatible(incompatible.advertisedLatest)
    ...
    actual suspend fun performUpdate(pluginId: String, manager: DynamicPluginManager): Result<String> {
        val mgr = PluginStoreSetup.updateManager
            ?: return Result.failure(Exception("Plugin store not initialized"))
        val update = mgr.availableUpdates.value.firstOrNull { it.pluginId == pluginId }

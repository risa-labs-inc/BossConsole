...
object AWTKeyboardInterceptor {
    ...
    internal fun processKeyEvent(
        ...
        // If the event was generated by AltGr (e.g. European layouts typing Polish 'ć' via AltGr+C),
        // it should NOT be intercepted as Ctrl+Alt shortcuts.
        ...
        val handled =
            when {
                event.keyCode == KeyEvent.VK_SHIFT -> handleShiftEvent(event, windowId)
                event.id == KeyEvent.KEY_RELEASED -> handleKeyReleased(event)
                event.id == KeyEvent.KEY_PRESSED -> windowId != null && handleKeyPressed(event, windowId)
                else -> false
            }
        ...
        when (actionId) {
            ...
            KeymapActions.BROWSER_COPY_MARKDOWN -> {
                ...
                if (!isModalDialogOpen() &&
                    ActiveBrowserRegistry.activeIn(windowId) != null &&
                    isBrowserComponentOrDescendant(focusOwner)
                ) {
                    if (perform) MenuActionsHandler.triggerBrowserCopyMarkdown(windowId)
                    true
                } else {
                    false
                }
            ...
            KeymapActions.BROWSER_COPY_LINK -> {
                val focusOwner = KeyboardFocusManager.getCurrentKeyboardFocusManager().focusOwner
                if (!isModalDialogOpen() &&
                    ActiveBrowserRegistry.activeIn(windowId) != null &&
                    isBrowserComponentOrDescendant(focusOwner)
                ) {
                    if (perform) MenuActionsHandler.triggerBrowserCopyLink(windowId)
                    true
                } else {
                    false
                }
    ...
    private fun isBrowserComponentOrDescendant(component: Component?): Boolean {
        if (java.awt.GraphicsEnvironment.isHeadless()) return true
        var current: Component? = component
        while (current != null) {
            if (current.javaClass.name.contains("jxbrowser", ignoreCase = true)) break
            current = current.parent
        }
        return current != null
    }

    private fun isModalDialogOpen(): Boolean {
        if (java.awt.GraphicsEnvironment.isHeadless()) return false
        val focusedWindow = KeyboardFocusManager.getCurrentKeyboardFocusManager().focusedWindow
        val isFocusedModal = focusedWindow is Dialog && focusedWindow.isModal
        val allWindows = Window.getWindows()
        val hasModalDialog = allWindows.any { it is Dialog && it.isModal && it.isVisible }
        return isFocusedModal || hasModalDialog
    }
}
